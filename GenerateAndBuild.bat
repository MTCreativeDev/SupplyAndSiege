...
@echo off
if /i "%~1" NEQ "__keep" (
  start "Hooks'-Generate&Build-Tool" cmd /k ""%~f0" __keep"
  exit /b
)
...
REM -- Check for VS 2022 with C++ build tools via vswhere
if exist "%VSWHERE%" (
    for /f "usebackq tokens=*" %%I in (`"%VSWHERE%" -version [17.0^,18.0^) -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property displayName 2^>nul`) do (
        set "HAS_VS=1"
        set "VS_DISPLAY_NAME=%%I"
    )
)

REM -- Check for Windows 10/11 SDK via registry
for /f "tokens=2,*" %%A in ('reg query "HKLM\SOFTWARE\Microsoft\Windows Kits\Installed Roots" /v KitsRoot10 2^>nul ^| find /i "REG_SZ"') do (
    set "KITS_ROOT=%%B"
)
if defined KITS_ROOT (
    REM Find the highest installed SDK version (e.g. 10.0.22621.0)
    for /f "delims=" %%V in ('dir /b /ad /on "%KITS_ROOT%Include\" 2^>nul ^| findstr /r "^10\."') do (
...
if "!HAS_VS!"=="0" if "!HAS_SDK!"=="0" (
    ...
    if /i "!INSTALL_TOOLS!" NEQ "n" (
        echo.
        echo [INFO] Installing VS 2022 Build Tools with C++ workload + Windows SDK...
...
if "!HAS_SDK!"=="0" (
    ...
    echo.
    set /p "INSTALL_SDK=Install Windows SDK now via winget? [Y/n]: "
    if /i "!INSTALL_SDK!" NEQ "n" (
        echo.
        echo [INFO] Installing latest Windows SDK...
        echo.
        winget install Microsoft.WindowsSDK.10.0.26100 --accept-source-agreements --accept-package-agreements
        if !ERRORLEVEL! NEQ 0 (
            echo [WARN] winget install failed. Trying via VS Build Tools modifier...
            winget install Microsoft.VisualStudio.2022.BuildTools --override "--wait --passive --add Microsoft.VisualStudio.Component.Windows11SDK.26100" --accept-source-agreements --accept-package-agreements
            if !ERRORLEVEL! NEQ 0 (
                echo.
        ...
        echo.
        echo [OK] Windows SDK installed. Continuing...
        echo.
    ) else (
        echo.
...
REM ---- Build via UnrealBuildTool directly (avoids Build.bat quirks)
...
REM ---- Find the engine-bundled dotnet.exe (UE uses versioned subfolders)
set "DOTNET_EXE="
...
REM ---- Verify it exists (should, but keeps error clear)
